...
import { Search, Filter, SlidersHorizontal, ChevronLeft, ChevronRight, UtensilsCrossed } from "lucide-react"
...
export function MenuItemList({ initialMenuItems = [] }: MenuItemListProps) {
  ...
  const buildSortBy = () => {
    ...
    return menuItemPaginateConfig.defaultSortBy[0][0] + ":" + menuItemPaginateConfig.defaultSortBy[0][1]
  }

  const { data: menuItemsResponse, isLoading } = useMenuItems({
    page,
  ...
  useEffect(() => {
    setPage(1)
  }, [itemTypeColumn, sortColumn, orderColumn, searchColumn])
    ...
    <div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-8 border border-gray-200 dark:border-gray-700">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          {/* Item Type Filter */}
          <Select value={itemTypeColumn || "ALL"} onValueChange={(val) => setItemTypeColumn(val === "ALL" ? "" : val)}>
            <SelectTrigger className="border-gray-300 dark:border-gray-600 focus:border-orange-500 focus:ring-orange-500">
        ...
        {(searchQuery || searchColumn || itemTypeColumn || sortColumn) && (
          <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            {itemTypeColumn && (
              <Button variant="secondary" size="sm" onClick={() => handleClearFilter("itemType")} className="h-7 text-xs">
                {menuItemPaginateConfig.filterableColumns.item_type.find(o => o.value === itemTypeColumn)?.label} ×
              </Button>
            )}
            {sortColumn && (
              <Button variant="secondary" size="sm" onClick={() => handleClearFilter("sort")} className="h-7 text-xs">
                Sắp xếp: {menuItemPaginateConfig.sortableColumns.find(c => c.value === sortColumn)?.label} {orderColumn === "ASC" ? "↑" : "↓"} ×
              </Button>
            )}
          </div>
        )}
